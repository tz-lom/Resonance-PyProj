...
class TestPipeTransformChannels(TestProcessor):
    def test_simple(self):
        si = resonance.si.Channels(2, 31, 10)
        db = resonance.db.Channels(si, 3e9, np.arange(0, 26))

        def transform(x):
            return x

        self.check_processor([si], [db], {'out_0': [db]},
                             resonance.pipe.transform_channels, 2, transform)
    ...
    def test_simple_computation(self):
        ...
        self.check_processor([si], [db], {'out_0': [db * scale]},
                             resonance.pipe.transform_channels, 1, transform)
    ...
    def test_change_number_of_channels(self):
        ...
        expected = resonance.db.Channels(si2, timestamp,
                                         np.concatenate((db, db * -0.5), 1))
