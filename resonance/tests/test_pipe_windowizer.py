...
class TestPipeWindowizer(TestProcessor):
    def test_one_by_one(self):
        ...
        self.check_processor(
            [si],
            blocks,
            {"out_0": [resonance.db.combine(e_db1, e_db2, e_db3)]},
            resonance.pipe.windowizer,
            5,
            1,
        )
    ...
    def test_from_two_blocks(self):
        ...
        self.check_processor(
            ...
            {
                "out_0": [
                    resonance.db.make_empty(e_si),
                    resonance.db.combine(e_db1, e_db2, e_db3),
                ]
            },
            resonance.pipe.windowizer,
            5,
    ...
    def test_1(self):
        si = resonance.si.Channels(1, 1)
        blocks = self.generate_channels_sequence(si, [4, 4, 10])

        e_si = resonance.si.Window(1, 5, 1)
        e_db = [
            resonance.db.Window(e_si, [], []),
            resonance.db.combine(
                self.generate_window(e_si, 0), self.generate_window(e_si, 2)
            ),
            resonance.db.combine(
                self.generate_window(e_si, 4),
                self.generate_window(e_si, 6),
                self.generate_window(e_si, 8),
                self.generate_window(e_si, 10),
                self.generate_window(e_si, 12),
            ),
        ]
